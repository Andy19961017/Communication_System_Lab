function binary_sequence = symbol_demapper(symbol_sequence, M, d, constellation, mapping, decision_rule)
    ...
    if ~strcmp(decision_rule,'MD')
        error('Invalid decision rule')
    end
    ...
    keys=dict.keys();
    key_size=size(keys,2);
    binary_sequence=[];
    for i=1:size(symbol_sequence,2)
        min_dist=Inf;
        min_dist_sym='';
        for k=1:key_size
            dist=abs(symbol_sequence(i)-d*dict{keys{k}});
            if dist<min_dist
                min_dist_sym=keys{k};
                min_dist=dist;
            end
        end
        binary_sequence=[binary_sequence,str2num(string(min_dist_sym))];
    end
    
    binary_sequence = dec2bin(binary_sequence(:),log2(M));
    binary_sequence=reshape(binary_sequence.',1,[])-'0';
end